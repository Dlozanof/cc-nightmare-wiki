...
## Clause V.11.3.4 - World modeling
...
## Clause V.11.3.5 - Reports
There are many kinds of reports while a mission is ongoing, but the most in depth one is the combat report.


### Clause V.11.3.5.1 - Combat Reports 
There are several events that trigger a combat, and the results of it will be reported to your desk at some point.

{{< callout emoji="🔧" text="Designer though: When to make the reports available? I would say that by the end of the missions so all of them appear together." >}}

Before explaining the report itself, the first topic to define is combat itself. It is automatic and every CC and creature will behave in its own interest. The way I see it whenever a combat happens there is a `combat field` divided in 6 slots:
...
**Initiative calculation example**
...
CC detects Yetis?
  - CC's perception: 5 (skill) + 5 (heat fingerprint detector) = 10
  - Yeti subterfuge: 6 (Skill) = 6
  - Dice are rolled in some TBD way and CC succeeds!!
Yetis detect CCs?
  - CC's subterfuge: 6 (skill) + 1 (trenchcoat) = 7
  - Yeti perception: 5 (skill) = 5
  - Dice are rolled in some TBD way and Yeti fails!!
```

Resulting in the following combat beginning:
```
| Team 1                               | Team 2
| Long range | Mid range | Melee range | Melee range | Mid range | Long range
               Yisus                                   Yeti
...
If both team had succeeded instead:
```
| Team 1                               | Team 2
| Long range | Mid range | Melee range | Melee range | Mid range | Long range
               Yisus                     Yeti
...
And if only the Yeti's suceeded: 
```
| Team 1                               | Team 2
| Long range | Mid range | Melee range | Melee range | Mid range | Long range
                           Yisus         Yeti
...
{{< callout emoji="✨" text="Random idea: On the beginning I was thinking that scaping would be a different phase, but instead it could just be a survival check after a unit is defeated. In this sense defeat is not killing but receiving enough damage that the unit deems success impossible and tries to scape." >}}

Combat rounds are simple. Every field sector units can only target to enemies in certain sectors of the opposing side:
...
If no target is available, the unit will move to a possition that will improve the chances to get a target. For example a melee unit that is surprised in `Mid range` will lose its first turn running to `Melee range` and the second turn to move the enemies in `Mid Range` to their own `Melee range`.

Everything in a turn happens simultaneously, and is resolved in the sectors that the units began their turn on. A unit that moves from `Mid range` to `Melee range` can be killed in the process, so the movement does not happen, but is targeted in `Mid range`, not `Melee range`.
